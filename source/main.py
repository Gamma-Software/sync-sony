...
def filter_files_in_subfolders(path, ext: list):
    files = []
...
def find_file_in_subfolders(path, filename, ext):
    for path in Path(path).rglob(filename+ext):
        return str(path)
...
def find_folders_in_subfolders(path, foldername):
    ...
    for root, dirs, filenames in os.walk(path):
        if foldername in dirs:
            print(os.path.join(root, foldername))
...
def convert_raw():
    ...
    if len(filepath_to_convert) > 0:
        ...
        for f in filepath_to_convert:
            ...
            # Convert arw to dng
            exit_code = os.system("docker run -v {}:{} raw2dng {}".format(BACKUP_SOURCE, BACKUP_SOURCE, f))
            print("    exit code: {}".format(exit_code))
            
            # If the conversion is unsuccessful, we rename the file to avoid converting it again
            if exit_code != 0:
                shutil.move(f, f.replace(".ARW", ".ARW.corrupted"))
            else:
    ...
    print("---- Step: convert dng to jpg")
    # Get the JPG generated by the camera and the converted folder
    converted_files_jpg = filter_files_in_subfolders(BACKUP_SOURCE, [".jpg", ".JPG"])
    [shutil.move(f, os.path.join(os.path.dirname(f), "converted", "processed", os.path.basename(f).replace(".JPG", ".jpg"))) for f in converted_files_jpg if "converted" not in f]
    converted_files_jpg = [os.path.splitext(os.path.basename(f))[0] for f in converted_files_jpg]
    
    # get the list of converted dng files
    converted_files_dng = filter_files_in_subfolders(BACKUP_SOURCE, [".dng"])
    converted_files_dng = [os.path.splitext(os.path.basename(f))[0] for f in converted_files_dng]
    
    # get the list of files to convert
    to_convert = list(set(converted_files_dng) - set(converted_files_jpg))
    filepath_to_convert = [find_file_in_subfolders(BACKUP_SOURCE, f, ".dng") for f in to_convert]
    ...
    if len(filepath_to_convert) > 0:
        ...
        for f in filepath_to_convert:
            ...
            # Convert dng to jpg and get the output code
...
def process_image():
    ...
    processed_files = filter_files_in_subfolders(BACKUP_SOURCE, ['_color.jpg'])
    ...
    filepath_to_process = [find_file_in_subfolders(BACKUP_SOURCE, f, '.jpg') for f in to_process]
    ...
    if len(filepath_to_process) > 0:
        ...
        for f in filepath_to_process:
            ...
            output_path = os.path.join(os.path.dirname(f))
            ...
            # cv2.imwrite(os.path.join(output_path, input_filename.replace('.jpg', '_norm.jpg')), image_norm)
            
            # im = Image.open(os.path.join(output_path, input_filename.replace('.jpg', '_norm.jpg')))
